...
eval({primop, Primop, Eis}, I, E, K, D, W, T) ->
  {Dis, MaxT} = ice_ctl:eval_seq(Eis, I, E, K, D, W, T),
...
eval({t, Es}, I, E, K, D, W, T) ->
  ...
  {Dis, MaxT} = ice_ctl:eval_seq(XiEis, I, E, K, D, W, T), %% XXX Does evaluating lhs make sense if dims are not ground values?
  case ice_sets:union_d(Dis) of
    {true, Dims} ->
      {Dims, MaxT};
    {false, Dis1} ->
...
eval({b_apply, E0, Eis}, I, E, K, D, W, T) ->
  {D0is, MaxT} = ice_ctl:eval_seq([E0 | Eis], I, E, K, D, W, T),
...
freeze_closure({closure, _ClI, _ClE, Abs}=Cl, I, E, K, D, W, T) ->
  ...
  {Dis, MaxT} = ice_ctl:eval_seq(Is, I, E, K, D, W, T),
  case ice_sets:union_d(Dis) of
    {true, Dims} ->
      {Dims, MaxT};
